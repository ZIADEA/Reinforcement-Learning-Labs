...
# (ton contenu existant inchangé ci-dessus)
import os
import numpy as np
import matplotlib.pyplot as plt
from typing import Dict, List, Tuple
...
# ... [tes fonctions existantes: policy_and_value_fig, summary_dashboard, plot_visit_heatmap,
#     plot_dominant_actions, plot_V_heatmap, plot_pi_policy] ...

# ===================== AJOUTS : plots d'historiques =====================
...
def plot_loss_history(ep_loss: np.ndarray, save_path: str, window: int = 10, title: str = "Loss moyenne par épisode"):
    """
    Trace la loss moyenne par épisode + moyenne mobile.
    """
    plt.figure(figsize=(8, 4))
    plt.plot(ep_loss, label="Loss moy/épisode")
    if ep_loss is not None and len(ep_loss) >= max(2, window):
        k = np.ones(window) / float(window)
        smooth = np.convolve(ep_loss, k, mode="valid")
        x = list(range(window, window + len(smooth)))
        plt.plot(x, smooth, linestyle="--", label=f"Moy. mobile ({window})")
    plt.title(title)
    plt.xlabel("Épisode"); plt.ylabel("MSE (Y - Q)^2")
    plt.grid(alpha=.3); plt.legend()
    _save_fig(save_path)
    return save_path

def plot_steps_history(ep_length: np.ndarray, save_path: str, title: str = "Nombre de steps jusqu'au goal"):
    """
    Trace le nombre de pas par épisode.
    """
    plt.figure(figsize=(8, 4))
    plt.plot(ep_length)
    plt.title(title)
    plt.xlabel("Épisode"); plt.ylabel("Steps")
    plt.grid(alpha=.3)
    _save_fig(save_path)
    return save_path

def plot_epsilon_theta_history(ep_epsilon: np.ndarray = None,
                               ep_theta_norm: np.ndarray = None,
                               save_path: str = "epsilon_theta.png",
                               title: str = "Exploration et norme des poids"):
    """
    Trace epsilon (si fourni) et ||θ|| (si fourni) sur le même graphe (axes partagés).
    """
    plt.figure(figsize=(8, 4))
    has_any = False
    if ep_epsilon is not None and len(ep_epsilon) > 0:
        plt.plot(ep_epsilon, label="epsilon"); has_any = True
    if ep_theta_norm is not None and len(ep_theta_norm) > 0:
        plt.plot(ep_theta_norm, label="||θ||"); has_any = True
    if not has_any:
        plt.close(); return None
    plt.title(title); plt.xlabel("Épisode")
    plt.grid(alpha=.3); plt.legend()
    _save_fig(save_path)
    return save_path

def plot_loss_vs_return(ep_loss: np.ndarray, ep_return: np.ndarray, save_path: str,
                        title: str = "Loss vs Return"):
    """
    Nuage de points loss moyenne/épisode vs retour/épisode (corrélation visuelle).
    """
    plt.figure(figsize=(6, 5))
    plt.scatter(ep_loss, ep_return, s=14, alpha=0.6)
    plt.title(title)
    plt.xlabel("Loss moy/épisode"); plt.ylabel("Return/épisode")
    plt.grid(alpha=.3)
    _save_fig(save_path)
    return save_path

def plot_training_curves(ep_return: np.ndarray,
                         ep_length: np.ndarray,
                         ep_loss: np.ndarray,
                         ep_epsilon: np.ndarray = None,
                         ep_theta_norm: np.ndarray = None,
                         save_dir: str = ".",
                         prefix: str = "") -> Dict[str, str]:
    """
    Wrapper pratique : génère 4 figures standards des historiques et renvoie leurs chemins.
      - Loss moyenne par épisode (+ moyenne mobile 10)
      - Steps par épisode
      - Epsilon & ||θ|| (si fournis)
      - Corrélation Loss vs Return
    """
    ensure_dir(save_dir)
    out = {}

    loss_path = os.path.join(save_dir, f"{prefix}loss_mean_per_episode.png")
    out["loss"] = plot_loss_history(ep_loss, loss_path, window=10,
                                    title="Loss moyenne par épisode")

    steps_path = os.path.join(save_dir, f"{prefix}steps_per_episode.png")
    out["steps"] = plot_steps_history(ep_length, steps_path,
                                      title="Nombre de steps jusqu'au goal")

    if (ep_epsilon is not None and len(ep_epsilon) > 0) or (ep_theta_norm is not None and len(ep_theta_norm) > 0):
        eps_theta_path = os.path.join(save_dir, f"{prefix}epsilon_theta_over_episodes.png")
        out["epsilon_theta"] = plot_epsilon_theta_history(ep_epsilon, ep_theta_norm, eps_theta_path,
                                                          title="Exploration et norme des poids")
    else:
        out["epsilon_theta"] = None

    lvret_path = os.path.join(save_dir, f"{prefix}loss_vs_return.png")
    out["loss_vs_return"] = plot_loss_vs_return(ep_loss, ep_return, lvret_path,
                                                title="Loss vs Return")
    return out
...
def plot_param_bars(layer_names, layer_totals, per_neuron_list, save_path, title="Taille des couches (nb paramètres)"):
    import matplotlib.pyplot as plt
    import numpy as np
    plt.figure(figsize=(8,4))
    x = np.arange(len(layer_names))
    plt.bar(x, layer_totals)
    for i,(tot,pn) in enumerate(zip(layer_totals, per_neuron_list)):
        plt.text(i, tot, f"{tot:,}\n({pn:,}/neur.)", ha="center", va="bottom", fontsize=8)
    plt.xticks(x, layer_names, rotation=20)
    plt.ylabel("Paramètres"); plt.title(title)
    plt.grid(axis="y", alpha=.25, linestyle="--", linewidth=.6)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    return save_path

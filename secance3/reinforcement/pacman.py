...
def readCommand(argv):
    ...
    if options.gameToReplay != None:
        ...
        try:
            ...
            print("Recorded game file '%s' not found.\nEither pass the full path to a recorded game file or run pacman with --record to generate recordings." % options.gameToReplay)
            sys.exit(1)
        except Exception as e:
            print("Error reading recorded game '%s': %s" % (options.gameToReplay, str(e)))
            sys.exit(1)
...
def replayGame(layout, actions, display):
    ...
    # Pass horizon (max timesteps) as the number of actions recorded
    horizon = len(actions)
    ...
    for i in range(numGames):
        ...
        if record:
            ...
            try:
                ...
                print("Failed to write recorded game to '%s': %s" % (fname, str(e)))
